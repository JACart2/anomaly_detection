...
4. LLM call using `llm_client.py`; a JSON artifact capturing the cached input data and model response is saved to `/root/dev_ws/src/anomaly_detection/logs` on each invocation (config.yaml -> api_artifact_output_dir)
5. LLM response standardization into `Decision` type using `response_handler.py`
6. Decision evaluation and alert publishing
7. `.bag` file of entire run context monitoring `raw_input_topic` if enabled in separate terminal.
